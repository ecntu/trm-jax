...
class SwiGLU(nnx.Module):
    ...
    def __init__(self, d_model, expansion, rngs):
        d_inter = _find_multiple(round(d_model * expansion * 2 / 3), 256)
        self.W1 = nnx.Linear(d_model, d_inter, use_bias=False, rngs=rngs)
        self.W3 = nnx.Linear(d_model, d_inter, use_bias=False, rngs=rngs)
        self.W2 = nnx.Linear(d_inter, d_model, use_bias=False, rngs=rngs)
...
class MixerBlock(nnx.Module):
    def __init__(self, seq_len, h_dim, expansion, rngs):
        self.l_mixer = SwiGLU(seq_len, expansion, rngs=rngs)
        self.d_mixer = SwiGLU(h_dim, expansion, rngs=rngs)
        self.l_norm = nnx.RMSNorm(h_dim, use_scale=True, rngs=rngs)
        self.d_norm = nnx.RMSNorm(h_dim, use_scale=True, rngs=rngs)
...
def Net(seq_len, h_dim, expansion, n_layers, rngs):
    return nnx.Sequential(
        lambda x, y, z: x + y + z,
        *[
            MixerBlock(seq_len=seq_len, h_dim=h_dim, expansion=expansion, rngs=rngs)
            for _ in range(n_layers)
        ],
        nnx.RMSNorm(h_dim, use_scale=True, rngs=rngs),
    )


class InitState(nnx.Module):
    def __init__(self, mode, batch_size, seq_len, h_dim, rngs):
        self.scale = jnp.sqrt(1 / h_dim)  # match (x) Emb's scale
...
# def loss_fn(
#     model, x, z, y, y_true, n: int = 6, T: int = 3, halt_loss_weight: float = 0.5
# ):
#     (y, z), y_hat, q_hat = model(x=x, y=y, z=z, n=n, T=T)

#     flat_mask = (rearrange(y_true, "b l -> (b l)") != -1).astype(jnp.float32)

#     # TODO change to dummy classs

#     rec_loss = softmax_ce(
#         rearrange(y_hat, "b l c -> (b l) c"),
#         rearrange(y_true, "b l -> (b l)"),
#     )
#     rec_loss = (rec_loss * flat_mask).sum() / (flat_mask.sum() + 1e-9)

#     halt_target = (y_hat.argmax(axis=-1) == y_true).astype(jnp.float32)
#     halt_loss = binary_ce(
#         q_hat,
#         (y_hat.argmax(axis=-1) == y_true).mean(axis=-1, keepdims=True),
#     )
...
def loss_fn(
    ...
    # forward
    (y, z), y_hat, q_hat = model(
        x=x, y=y, z=z, n=n, T=T
    )  # y_hat: (B, L, C); q_hat: (B, 1)

    # ----- masked cross-entropy over padded tokens -----
    # mask True where labels are valid
    mask = y_true != -1  # (B, L) bool
    mask_f = mask.astype(jnp.float32)  # (B, L) float

    # replace ignored labels with a dummy class id (0), then mask the loss
    labels_safe = jnp.where(mask, y_true, 0)  # (B, L) int

    logits_flat = rearrange(y_hat, "b l c -> (b l) c")
    labels_flat = rearrange(labels_safe, "b l -> (b l)")
    mask_flat = rearrange(mask_f, "b l -> (b l)")

    per_pos_ce = softmax_ce(logits_flat, labels_flat)  # (B*L,)
    rec_loss = (per_pos_ce * mask_flat).sum() / (mask_flat.sum() + 1e-9)

    # ----- halt head target = masked per-example accuracy -----
    preds = y_hat.argmax(axis=-1)  # (B, L)
    correct = (preds == y_true) & mask  # (B, L) bool
    num_correct = correct.sum(axis=-1)  # (B,)
    num_valid = mask_f.sum(axis=-1)  # (B,)
    acc_per_ex = num_correct / jnp.maximum(num_valid, 1.0)  # (B,)

    halt_loss = binary_ce(q_hat, acc_per_ex[..., None]).mean()
...
def train_step(
    grad_fn, model, ema_model, opt, batch, ema_beta, N_supervision, rngs, step
):
    model.train()
    ...
    # ema only once -- IDEA: could at every sup step
    do_update = (
        step >= 100
    )  # & ((step % 10) == 0)  # bool, JAX-traced TODO remove if unnecessary
    ema_step_size = (1.0 - ema_beta) * do_update.astype(jnp.float32)
    new_ema_model = optax.incremental_update(model, ema_model, step_size=ema_step_size)

    metrics = {f"train/loss@{i}": losses[i] for i in range(0, N_supervision, 2)}
    metrics["train/loss"] = losses[-1]
    metrics["train/grad_norm"] = norms[-1]
    return model, opt, new_ema_model, metrics  # TODO ema inside train_step?


@nnx.jit(static_argnames=("n", "T", "N_supervision"))
def eval_step(model: TRM, batch, n, T, N_supervision, rngs):
    model.eval()
...
def evaluate(model: TRM, data_iter: Dataset, eval_step):
...
class Loader:
    def __init__(
        ...
        shuffle_seed=None,
        prefetch_size=2,
        pad_to_seq_len=None,  # pad to multiple of 8 for better utilization
    ):
    ...
    def pad_inputs(self, batch):
        ...
        if self.pad_to_seq_len or 0 > x.shape[1]:
            pad_len = self.pad_to_seq_len - x.shape[1]
            if pad_len > 0:
                x = np.pad(x, ((0, 0), (0, pad_len)), constant_values=0)
                y = np.pad(y, ((0, 0), (0, pad_len)), constant_values=-1)
        return x, y
    ...
    def iter_batch(self):
        ...
        while epochs is None or epochs > 0:
            for i in range(0, N, bs):
                ...
                x, y = self.pad_inputs(batch)
                yield {
                    "inputs": np.asarray(x, dtype=np.int32),
...
if __name__ == "__main__":
    ...
    train_loader = Loader(
        train_ds, batch_size=args.batch_size, epochs=args.epochs, shuffle_seed=seed
    )
    ...
    model = TRM(
        ...
        output_head=nnx.Linear(args.h_dim, vocab_size, dtype=dtype, rngs=rngs),
        Q_head=nnx.Sequential(
            partial(reduce, pattern="b l h -> b h", reduction="mean"),
            nnx.Linear(args.h_dim, 1, dtype=dtype, rngs=rngs),
        ),
        input_embedding=nnx.Embed(vocab_size, args.h_dim, dtype=dtype, rngs=rngs),
        init_y=InitState(
            "random",
            args.batch_size,
            seq_len,
    ...
    n_params = sum(
        jax.tree.map(
            jnp.size, jax.tree.leaves(nnx.filter_state(nnx.state(model), nnx.Param))
        )
    )
    print(f"No. of parameters: {n_params}")

    num_train_steps = args.steps or args.epochs * (len(train_ds) // args.batch_size)
    args.steps = num_train_steps  # for logging
    ...
    writer.write_hparams(vars(args))  # if start_step == 0
    ...
    if args.workdir is not None and jax.process_index() == 0:
        hooks.append(
            periodic_actions.Profile(num_profile_steps=5, logdir=args.workdir),
        )

    with metric_writers.ensure_flushes(writer):
        for step, batch in enumerate(train_loader, start=1):
            model, opt, ema_model, metrics = train_step(
                grad_fn,
                model,
                ema_model,
                opt,
    ...
    # TODO:
    # - add checkpointing
    # - inc utilization -> pad to mults of 8, bf16, etc.
